...
type PhraseStepProps = {
  ...
  onNext: () => void;
  onRegenerate: () => void;
  onOpenWhereFrom: () => void;
};

export function PhraseStep({ words, onNext, onRegenerate, onOpenWhereFrom }: PhraseStepProps) {
  const [revealAll, setRevealAll] = useState(false);
  const [heldIndex, setHeldIndex] = useState<number | null>(null);

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Here&rsquo;s your new seed phrase</Text>
      <Text style={styles.body}>
        {words.length} words, in this order. Hold a word to reveal it — it hides again when you
        let go.
      </Text>

      <View style={styles.warningBox}>
        <Text style={styles.warningText}>
          Don&rsquo;t screenshot this and don&rsquo;t let anyone photograph it. Whoever has all
          these words has access to your money.
        </Text>
      </View>
      ...
      <View style={styles.toolbarRow}>
        ...
        <Pressable onPress={onOpenWhereFrom} hitSlop={8} style={styles.whereFromLink}>
          <Text style={styles.whereFromText}>Where&rsquo;s this from?</Text>
        </Pressable>
      </View>
      ...
      <Pressable onPress={onNext} style={styles.cta}>
        <Text style={styles.ctaText}>Back it up first</Text>
      </Pressable>
      <Pressable onPress={onRegenerate} hitSlop={8} style={styles.regenerateLink}>
        <Text style={styles.regenerateText}>Make a different one</Text>
      </Pressable>
    </View>
  );
}
